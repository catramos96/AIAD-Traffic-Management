...
import cityTraffic.onto.ServiceExecutionRequest;
import cityTraffic.onto.ServiceOntology;
import cityTraffic.onto.ServiceProposal;
...
public class ProviderAgent extends Agent {
	...
	private class RequestInit extends AchieveREInitiator {
		...
		public void onStart() {
			...
			Agent agent = CityTrafficBuilder.getAgent(context, subContractedProvider);
			net = (Network<Object>) ContextUtils.getContext(myAgent).getProjection("Service Consumer/Provider network");
			edge = net.addEdge(myAgent, agent);
		}
		...
		public int onEnd() {
			// remove edge
			if(edge != null && net.containsEdge(edge)) {
				net.removeEdge(edge);
			}
